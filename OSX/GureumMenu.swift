    ...
    UpdateManager.shared.requestVersionInfo(mode: mode) { info in
      ...
      if info.update.version == info.current {
        ...
        if let url = URL(string: info.update.url) {
          alert.addButton(withTitle: "업데이트 받기")
